...
from item_ranker.io.factory_rdd import *
...
def broadcast_dataset_b(spark, dataset_b_rdd):
    ...
    Args:
        ...
        locations_rdd: RDD of (geographical_location_oid, geographical_location).

    Returns:
        A Spark broadcast variable wrapping {geo_oid: geo_location} dict.
    """
    locations_dict = dict(dataset_b_rdd.map(tuple).map(lambda row: (row[0], row[1])).collect())
...
def run(spark, config: PipelineConfig):
    ...
    dataset_a_rdd = RDDIOFactory.read_rdd(spark, PARQUET_FORMAT, config.dataset_a_path )
    dataset_b_rdd = RDDIOFactory.read_rdd(spark, PARQUET_FORMAT, config.dataset_b_path )
    
    logger.info("Loaded detections dataset with rows=%s", dataset_a_rdd.count())
    logger.info("Loaded locations dataset with  rows=%s", dataset_b_rdd.count())
    ...
    geo_oid_idx   = DATASETA_SCHEMA.fieldNames().index("geographical_location_oid")
