...
class RDDWriter(ABC):
    def __init__(self, is_overwrite:bool, run_date: Optional[str] = None):
        """Initialize writer mode and run date, defaulting run_date to today."""
    ...
    def _dated_output_path(self, base_path: str) -> str:
        """Return output path with run_date, inserting before filename when present."""
        ...
        has_file_extension = "." in leaf and not leaf.startswith(".") and not leaf.endswith(".")
...
class CSVRDDWriter(RDDWriter):
    def write(self, sc, rdd, path, schema):
        """Convert an RDD  to a DataFrame and write as csv file organised by date folder.."""
...
class JSONRDDWriter(RDDWriter):
    def write(self, sc, rdd, path, schema):
        """Convert an RDD to a DataFrame and write as JSON String organised by date folder.."""
...
class ParquetRDDWriter(RDDWriter):
    def write(self, sc, rdd, path, schema):
        """Convert an RDD to a DataFrame and write as Parquet organised by date folder.."""
